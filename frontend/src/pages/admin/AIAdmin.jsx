...
import {
  Bot, BarChart2, Settings, MessageSquare, TrendingUp,
  RefreshCw, Trash2, Save, ToggleLeft, ToggleRight,
  Loader2, AlertCircle, CheckCircle, Users, Zap,
  HelpCircle, GraduationCap, Calendar, Award, Microscope,
  Eye, EyeOff
} from 'lucide-react'
import {
...
const SOURCE_TOGGLES = [
  { key: 'use_programmes',   label: 'Programmes',   icon: GraduationCap, desc: 'AI answers questions about courses and programmes' },
  { key: 'use_faqs',         label: 'FAQs',         icon: HelpCircle,    desc: 'AI uses published FAQs to answer common questions' },
  { key: 'use_timetable',    label: 'Timetable',    icon: Calendar,      desc: 'AI can look up class schedules and room info' },
  { key: 'use_research',     label: 'Research',     icon: Microscope,    desc: 'AI references active research projects' },
  { key: 'use_scholarships', label: 'Scholarships', icon: Award,         desc: 'AI surfaces active scholarship opportunities' },
  { key: 'use_external_sources', label: 'External Sources', icon: Globe,  desc: 'AI uses crawled website pages and uploaded PDF documents' },
]
...
export default function AIAdmin() {
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    try {
      const [statsRes, topicsRes, volumeRes] = await Promise.allSettled([
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      const res = await api.get('/aiconfig/settings/')
  ...
  const handleSaveSettings = async () => {
    setSaving(true)
    try {
      await api.put('/aiconfig/settings/', settings)
  ...
  const handleDeleteSource = async (id) => {
    if (!window.confirm('Delete this source and all its indexed chunks?')) return
    try {
    ...
    <div className="p-4 md:p-8 min-h-screen bg-slate-50">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => { loadAnalytics(); loadSettings() }}
                ...
                {statsLoading ? (
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <LineChart data={volume}>
                      ...
                      <Tooltip
                        labelFormatter={d => new Date(d).toLocaleDateString('en-KE')}
                        formatter={(v) => [v, 'Chats']}
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={topics} layout="vertical">
                      ...
                      <YAxis
                        ...
                        dataKey="topic"
                        tick={{ fontSize: 10, fill: '#94a3b8' }}
                        tickFormatter={t => TOPIC_LABELS[t] || t}
                        width={80}
                      />
                      <Tooltip
                        formatter={(v) => [v, 'Chats']}
                        labelFormatter={t => TOPIC_LABELS[t] || t}
                        contentStyle={{ fontSize: 12, borderRadius: 8 }}
                      />
        ...
        {tab === 'settings' && (
          <div className="space-y-6">
            {settingsLoading ? (
              ...
              <div className="bg-red-50 border border-red-100 rounded-xl p-6 text-center text-red-500 text-sm">
              ...
              <>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-6">
                  <div className="mb-5">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      Control which sections of your website the AI can reference when answering student questions.
                    </p>
                  </div>
                  <div className="space-y-4">
                    {SOURCE_TOGGLES.map(({ key, label, icon: Icon, desc }) => (
                      <div key={key} className="flex items-center justify-between py-3 border-b border-slate-50 last:border-0">
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-lg ${settings[key] ? 'bg-[#1a2b4c]' : 'bg-slate-100'}`}>
                            <Icon size={15} className={settings[key] ? 'text-orange-400' : 'text-slate-400'} />
                          </div>
                          <div>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-6">
                  ...
                  <p className="text-xs text-slate-400 mb-4">
                    The first message students see when they open the chat widget.
                  </p>
                  <textarea
                    value={settings.greeting_message}
                    onChange={(e) => handleSettingChange('greeting_message', e.target.value)}
                    rows={3}
                    maxLength={500}
                    className="w-full border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-[#1a2b4c]/20 outline-none resize-none"
                    placeholder="Hello! 👋 I'm your Zetech University assistant..."
                  />
                  <div className="text-xs text-slate-400 text-right mt-1">
                    {settings.greeting_message?.length || 0}/500
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-6">
                  ...
                  <p className="text-xs text-slate-400 mb-4">
                    Override the default AI personality and instructions. Leave blank to use the built-in prompt.
                  </p>
                  {showPrompt && (
                    <textarea
                      value={settings.custom_system_prompt}
                      onChange={(e) => handleSettingChange('custom_system_prompt', e.target.value)}
                      rows={8}
                      className="w-full border border-slate-200 rounded-xl p-3 text-sm font-mono focus:ring-2 focus:ring-[#1a2b4c]/20 outline-none resize-none"
                      placeholder="You are a helpful assistant for Zetech University..."
                    />
                  )}
                  {!showPrompt && (
                    <div className="bg-slate-50 rounded-xl p-3 text-xs text-slate-400 italic">
                      {settings.custom_system_prompt
                        ? `Custom prompt set (${settings.custom_system_prompt.length} chars)`
                ...
                {/* Temperature */}
                <div className="bg-white rounded-2xl border border-slate-100 p-6">
                  <h3 className="font-bold text-slate-800 mb-1">AI Temperature</h3>
                  <p className="text-xs text-slate-400 mb-4">
                    Lower = more factual and consistent. Higher = more creative but less predictable.
                    Recommended: 0.7
                  </p>
                  <div className="flex items-center gap-4">
                    <span className="text-xs text-slate-400 w-16">Factual (0)</span>
                    <input
                      type="range"
                      min="0"
                      max="1"
                      ...
                      className="flex-1 accent-[#1a2b4c]"
                    />
                    <span className="text-xs text-slate-400 w-16 text-right">Creative (1)</span>
                    <span className="text-sm font-black text-[#1a2b4c] w-8 text-right tabular-nums">
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    disabled={saving}
                    className="flex items-center gap-2 px-6 py-3 bg-[#1a2b4c] hover:bg-slate-800 text-white rounded-xl font-bold transition-all disabled:opacity-50"
                  >
                    {saving
        ...
        {tab === 'logs' && (
          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-slate-800">Recent Chat Logs</h3>
                <p className="text-xs text-slate-400 mt-0.5">Latest 20 student questions</p>
              </div>
              <button
                onClick={loadLogs}
            ...
            {logsLoading ? (
              ...
              <div className="py-16 text-center text-slate-400 italic text-sm">
                No chat logs yet. Once students start chatting, their questions will appear here.
              </div>
            ) : (
                  ...
                  <div key={log.id} className="p-5 hover:bg-slate-50 transition-colors group">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          <span
                            ...
                            style={{
                              backgroundColor: `${TOPIC_COLORS[log.topic]}20`,
                              color: TOPIC_COLORS[log.topic]
                            }}
                          >
                            {TOPIC_LABELS[log.topic] || log.topic}
                          </span>
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase ${
                            ...
                            {log.was_helpful ? 'Helpful' : 'Failed'}
                          </span>
                          {log.had_timetable_results && (
                            <span className="text-[10px] bg-orange-100 text-orange-600 font-bold px-2 py-0.5 rounded-full uppercase">
                              Timetable
                            </span>
                          )}
                          {log.had_programme_results && (
                            <span className="text-[10px] bg-blue-100 text-blue-600 font-bold px-2 py-0.5 rounded-full uppercase">
                              Programmes
                            </span>
                          )}
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 p-1.5 text-red-400 hover:bg-red-50 rounded-lg transition-all flex-shrink-0"
                        title="Delete log"
                      >
                        <Trash2 size={14} />
                      </button>
            ...
            {sourcesLoading ? (
              ...
              <div className="bg-white rounded-2xl border border-slate-100 py-16 text-center">
                ...
                <p className="text-slate-300 text-xs mt-1">
                  Add a website URL or upload a PDF to get started.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                    ...
                    <div key={source.id}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 text-xs text-slate-400 mt-1">
                          ...
                          {source.source_type !== 'pdf' && (
                            <span>Depth: {source.crawl_depth}</span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        {source.source_type !== 'pdf' && (
                          <button onClick={() => handleCrawlNow(source.id)}
                            disabled={isLoading}
                            title="Re-crawl now"
